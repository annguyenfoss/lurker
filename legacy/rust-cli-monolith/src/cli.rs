...
use crate::model::{ColorMode, SourceKind, VolumeType};
        ...
        "create" => {
            ...
            if action_args.is_empty() || action_args.len() > 2 {
                return Err(usage_failure(
                    &script_name,
                    color_mode,
                    "Usage: lurker create [--type TYPE] <file> <size-gb> [-F|--force] or lurker create [--type TYPE] <block-device> [-F|--force]",
                ));
            }
                ...
                SourceKind::Block => {
                    if action_args.len() != 1 {
                        return Err(usage_failure(
                            &script_name,
                            color_mode,
                            "Usage: lurker create [--type TYPE] <block-device> [-F|--force]",
                        ));
                    }
                    None
                }
                SourceKind::File => {
                    if action_args.len() != 2 {
                        return Err(usage_failure(
                            &script_name,
                            color_mode,
                            "Usage: lurker create [--type TYPE] <file> <size-gb> [-F|--force]",
                        ));
                    }
...
fn reject_removed_operand_flag(action: &str, script_name: &str) -> String {
    match action {
        "create" => format!(
            "Operand flags are no longer supported. Use: {script_name} create [--type TYPE] <file> <size-gb> [-F|--force], {script_name} create [--type TYPE] <block-device> [-F|--force], or {script_name} createvc ..."
        ),
        "mount" => format!(
